...
class C_configuracion extends CI_Controller
{
    ...
    function C_gestionar_sistema($btn){
        ...
        if ($data[0]->oboolean == 't' && $btn == 'add') {
            ...
            if ($data[0]->oboolean == 't') {
                ...
                if ($data[0]->oboolean == 't') {
                    $data = $this->C_sincronizar_catalogos();
                }
            }
        }
        echo json_encode($data);
    ...
    function C_generar_cufd(){
        ...
        $facturacion    = $this->control->M_informacion_facturacion();
        $datos_cuis     = $this->control->M_datos_iniciales_cuis();        
        $Codigos        = new Codigos($facturacion);
    ...
    function C_sincronizar_catalogos(){

        $facturacion    = $this->control->M_credenciales_facturacion(0);
