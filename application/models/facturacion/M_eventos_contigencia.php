...
class M_eventos_contigencia extends CI_Model {
    ...
    function listar_cufd($fecha_inicial,$fecha_fin){
        $idlogin = $this->session->userdata('id_usuario');
        $query = $this->db->query("SELECT ROW_NUMBER() OVER (ORDER BY feccre desc )::INTEGER as nro,
                                    oc.id_cufd, 
                                    oc.cod_cufd ,oc.feccre, oc.fecven  FROM ope_cufd oc 
                                    ...
                                    and apiestado in ('PRE-ACTIVO','INACTIVO')
    ...
    function get_cufd_evento($idcufd){
    ...
    public function get_lst_eventos($id_cufd) {
        $user = $this->session->userdata('usuario');
    ...
    join cat_tipo_eventos cte
        on cte.id_tipo_evento = oe.id_tipo_evento and oe.usucre ilike '$user' and oe.tipo_contingencia = 0 and oe.id_cufd = $id_cufd;");
        return $query->result();
    }
}
